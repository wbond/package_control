    ...
    def download(self, url, error_message, timeout, tries, prefer_cached=False):
        ...
        while tries > 0:
            ...
            try:
                ...
                with open(self.tmp_file, 'r', encoding='utf-8', errors='replace') as f:
                    headers_str = f.read()
                self.clean_tmp_file()
        ...
        for section in sections:
            console_write(
                ...
                Curl HTTP Debug %s
                  %s
                ''',
                (section['type'], section['contents'].replace("\n", "\n  "))
            )
    ...
    def split_debug(self, string):
        ...
        for line in string.splitlines():
            ...
            if section != last_section:
                debug_section = debug_section.rstrip()
                if debug_section:
                    debug_sections.append({
                        'type': section,
                        'contents': debug_section
                    })
                    debug_section = ''
